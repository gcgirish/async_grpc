...
Rpc::async_reader_interface() {
  switch (rpc_handler_info_.rpc_type) {
    ...
    case ::grpc::internal::RpcMethod::CLIENT_STREAMING:
      return server_async_reader_.get();
    case ::grpc::internal::RpcMethod::NORMAL_RPC:
      std::cout << "For NORMAL_RPC no streaming reader interface exists.";
    case ::grpc::internal::RpcMethod::SERVER_STREAMING:
      std::cout << "For SERVER_STREAMING no streaming reader interface exists.";
  }
  std::cout << "Never reached.";
}
...
Rpc::async_writer_interface() {
  switch (rpc_handler_info_.rpc_type) {
    ...
    case ::grpc::internal::RpcMethod::CLIENT_STREAMING:
    case ::grpc::internal::RpcMethod::NORMAL_RPC:
      std::cout << "For NORMAL_RPC and CLIENT_STREAMING no streaming writer "
                    "interface exists.";
      break;
    case ::grpc::internal::RpcMethod::SERVER_STREAMING:
      return server_async_writer_.get();
...
Rpc::CompletionQueueRpcEvent* Rpc::GetRpcEvent(Event event) {
  switch (event) {
    ...
    case Event::WRITE_NEEDED:
      std::cout << "Rpc does not store Event::WRITE_NEEDED.";
      break;
    case Event::WRITE:
      return &write_event_;
    case Event::FINISH:
